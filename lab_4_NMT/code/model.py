...
class seq2seqAtt(nn.Module):
    ...
    def forward(self,target_h,source_hs):
        ...
        concat_output = self.ff_concat(torch.cat([target_h_rep, source_hs], dim=-1)).tanh()
...
class Decoder(nn.Module):
    ...
    def forward(self, input, source_context, h):
        ...
        tar_inp = self.embedding(input)
        out, h = self.rnn(tar_inp, h)
        tilde_h = torch.tanh(self.ff_concat(torch.cat((source_context, out), dim=-1)))
...
class seq2seqModel(nn.Module):
    ...
    ARGS = ['vocab_s','source_language','vocab_t_inv','vocab_s_inv','embedding_dim_s','embedding_dim_t',
     'hidden_dim_s','hidden_dim_t','hidden_dim_att','do_att','padding_token',
     'oov_token','sos_token','eos_token','max_size']
    def __init__(self, vocab_s, source_language, vocab_t_inv, vocab_s_inv, embedding_dim_s, embedding_dim_t, 
                 hidden_dim_s, hidden_dim_t, hidden_dim_att, do_att, padding_token,
                 oov_token, sos_token, eos_token, max_size):
    ...
    def forward(self,input,max_size,is_prod):
        ...
        while True:
            ...
            align.append(alpha.detach().numpy().reshape(-1))

            
            # fill the gap #
            # get the next input to pass the decoder
            target_input = torch.argmax(prediction, dim=-1)
    ...
    def sourceInts_to_nl(self,source_ints):
        '''converts integer target sentence into target natural language'''
        return ['<PAD>' if elt==self.padding_token else '<OOV>' if elt==self.oov_token \
                else '<EOS>' if elt==self.eos_token else '<SOS>' if elt==self.sos_token\
                else self.vocab_s_inv[elt] for elt in source_ints]


    
    def predict(self,source_nl):
        source_ints = self.sourceNl_to_ints(source_nl)
        logits, align = self.forward(source_ints,self.max_size,True) # (seq) -> (<=max_size,vocab)
        target_ints = logits.argmax(-1).squeeze() # (<=max_size,1) -> (<=max_size)
        min_size = min(len(align), 7)
        target_nl = self.targetInts_to_nl(target_ints.tolist())
        hm = pd.DataFrame(np.array(align[:min_size]), index =target_nl[:min_size], 
                                              columns = self.sourceInts_to_nl(source_ints.tolist()))
        ax = sns.heatmap(hm)
    ...
    def load(cls,path_to_file, vocab_source_inv):
        attrs = torch.load(path_to_file, map_location=lambda storage, loc: storage) # allows loading on CPU a model trained on GPU, see https://discuss.pytorch.org/t/on-a-cpu-device-how-to-load-checkpoint-saved-on-gpu-device/349/6
        state_dict = attrs.pop('state_dict')
        attrs.update({'vocab_s_inv': vocab_source_inv})
